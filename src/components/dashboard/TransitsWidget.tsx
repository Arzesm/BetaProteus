...
import { calculateTransitsForNow, TransitAspect as BaseTransitAspect } from "@/lib/transits";
...
interface TransitAspect extends BaseTransitAspect {
  ...
  useEffect(() => {
    if (chart) {
      const calculateTransits = async () => {
        ...
        try {
          const rawAspects = await calculateTransitsForNow(chart.chart_data);
          const enriched: TransitAspect[] = rawAspects.map((aspect) => ({
            ...aspect,
            interpretation: aspectDetails[aspect.aspectName]?.interpretation || "",
          }));
          setTransits(enriched);
        } catch (error) {
