...
import { MBTIQuiz } from "@/components/psychology/MBTIQuiz";
...
const PsychologyPage = () => {
  ...
  const tests = [
    {
      ...
      longDescription: 'MBTI — индикатор типа Майерс–Бриггс. Он помогает понять, как вы черпаете энергию (E/I), обрабатываете информацию (S/N), принимаете решения (T/F) и структурируете жизнь (J/P). Тест не оценивает «хорошо/плохо», а описывает предпочтения и сильные стороны.',
      icon: Layers,
      component: MBTIQuiz,
  ...
  const handleTestComplete = (testId: string, result: string) => {
  ...
  const getStatus = (testId: string) => {
    if (activeTest === testId) {
      return { icon: '🕓', text: 'В процессе', className: 'text-blue-600 bg-blue-100 dark:bg-blue-900/30 dark:text-blue-400' };
    }
    if (testResults[testId]) {
      return { icon: '✅', text: 'Пройден', className: 'text-green-600 bg-green-100 dark:bg-green-900/30 dark:text-green-400' };
    }
    return { icon: '⬜', text: 'Не пройден', className: 'text-gray-600 bg-gray-100 dark:bg-gray-700/30 dark:text-gray-400' };
  };
                ...
                >
                  <AccordionItem value={test.id} className="border-b-0">
                    ...
                    <AccordionContent className="px-6 pb-6 pt-2 space-y-4">
                      ...
                      {activeTest === test.id ? (
                        <test.component 
                          onComplete={(result: string) => handleTestComplete(test.id, result)}
                        />
                      ) : testResults[test.id] ? (
